...
def render_segmentation_cv2(segmentation_result: np.ndarray, color_list: np.ndarray):
    ...
    patter = np.zeros([height, width, 3])
    # fill color
    for class_id, color in enumerate(color_list):
        mask = (segmentation_result == class_id)
        # print(class_id, color, np.sum(mask), mask.shape)
        patter[mask] = color
    return patter.astype(np.uint8)
...
def freeze_model(model, freezing=True):
...
def get_param_groups_1(model):
    names_encoder, names_decoder = [], []
    params_encoder, params_decoder = [], []
    for name, param in model.named_parameters():
        if 'encode' in name or 'backbone' in name:
            params_encoder.append(param)
            names_encoder.append(name)
        else:
            params_decoder.append(param)
            names_decoder.append(name)
    params = [
        {"params": params_encoder},
        {"params": params_decoder}
    ]
    logging.info(f'encoder params: {names_encoder}')
    logging.info(f'decoder params: {names_decoder}')
    return params


def get_param_groups(model: nn.Module):
    wd_params, nwd_params, heads_wd_params, heads_nwd_params = [], [], [], []
    backbones = []
    for name, p in model.named_parameters():
        if p.dim() == 1:
            if 'backbone' in name or 'encoder' in name:
                ...
                heads_nwd_params.append(p)
        else:
            if 'backbone' in name or 'encoder' in name:
                wd_params.append(p)
                backbones.append(name)
            else:
                heads_wd_params.append(p)
    # logging.info(f'backbones params: {backbones}')
    params = [
        {"params": wd_params},
        {"params": nwd_params, "weight_decay": 0},
        {"params": heads_wd_params},
        {"params": heads_nwd_params, "weight_decay": 0}
    ]
    return params
...
def show_img_tensor(img_tensor, window_name, mean_rgb, std_rgb, multi=1.0, block=False):
    _mean = torch.Tensor(mean_rgb).unsqueeze(dim=-1).unsqueeze(dim=-1)
    _std = torch.Tensor(std_rgb).unsqueeze(dim=-1).unsqueeze(dim=-1)
    img_tensor = (img_tensor * _std + _mean) * multi
