...
class BaseData(Dataset):
    def __init__(self, image_dir, mask_dir, transforms=None,
                 ...
                 postfix_image='.png', postfix_label='.png'):
        assert label_type in ['id', 'prob']
        self.postfix_image = postfix_image
        ...
        if isinstance(image_dir, list):
            for img_dir_path, mask_dir_path in zip(image_dir, mask_dir):
                logger.info(f'loading from {image_dir} {mask_dir}')
                self.batch_generate(img_dir_path, mask_dir_path)
        else:
            self.batch_generate(image_dir, mask_dir)

        logger.info(f'All Dataset images: %d' % len(self.rgb_filepath_list))
        self.file_to_idx = {os.path.basename(file_path): idx for idx, file_path in enumerate(self.rgb_filepath_list)}
        self.transforms = transforms

    def batch_generate(self, image_dir, mask_dir):
        rgb_filepath_list = glob.glob(os.path.join(image_dir, f'*{self.postfix_image}'))
        ...
        for fname in rgb_filename_list:
            if mask_dir is not None:
                cls_filepath_list.append(os.path.join(mask_dir, fname.replace(self.postfix_image, self.postfix_label)))
            # a = os.path.join(mask_dir + '_sup', f"{fname.split('.')[0]}.tif")
            sup_filepath_list.append(os.path.join(image_dir.replace('img_dir', 'reg_dir')
                                                  .replace('images_png', 'reg_dir'),
                                                  f"{fname.split('.')[0]}.tif"))
    ...
    def __getitem__(self, idx):
        ...
        if len(self.cls_filepath_list) > 0:
            ...
            if self.read_sup:
                return image, dict(cls=mask.long(), sup=mask_sup, fname=os.path.basename(self.rgb_filepath_list[idx]))
            else:
                return image, dict(cls=mask.long(), fname=os.path.basename(self.rgb_filepath_list[idx]))
        else:
            if self.transforms is not None:
                # print(type(self), self.read_sup, mask_sup is None, self.rgb_filepath_list[idx])
                blob = self.transforms(image=image, mask_sup=mask_sup if self.read_sup else None)
                image = blob['image']
                if self.read_sup:
                    mask_sup = blob['mask_sup']
            if self.read_sup:
